...
public class CallDetailActivity extends Activity implements ProximitySensorAware {
    ...
    //BOWAY BEGIN qianweiqiang add  2016
    private Button  ringToneSelector;
    private TextView  ringtone;
    public static final int RINGTONE_PICKED = 35; 
    private boolean isNumberContact = false;
    private LinearLayout mDeleteCallLog;    
    //END BOWAY 
    ...
    private void updateData(final Uri... callUris) {
        class UpdateContactDetailsTask extends AsyncTask<Void, Void, PhoneCallDetails[]> {
            ...
            public void onPostExecute(PhoneCallDetails[] details) {
                ...
                mDeleteCallLog = (LinearLayout)findViewById(R.id.buttom_action_tool);
                ...
                mDeleteCallLog.setOnClickListener(new View.OnClickListener() {  
                    ...
                    public void onClick(View v) {
                            ...
                            public void onClick(DialogInterface dialog, int which) {
                                ...
                                mAsyncTaskExecutor.submit(Tasks.REMOVE_FROM_CALL_LOG_AND_FINISH,
                                        new AsyncTask<Void, Void, Void>() {
                                            @Override
                                            public Void doInBackground(Void... params) {
                                                CallDetailActivity.this.getContentResolver().delete(Calls.CONTENT_URI_WITH_VOICEMAIL,
                        ...
                        }).setNegativeButton(R.string.menu_doNotSave,new DialogInterface.OnClickListener() { 
                            @Override  
                            public void onClick(DialogInterface dialog, int which) {
                                // TODO Auto-generated method stub      
                               dialog.dismiss();
...
/*
                ...
                ringToneSelector = (Button)findViewById(R.id.call_log_detail);
                //ringtone = (TextView)findViewById(R.id.ringtone);


                if (!isNumberContact) {       
                String ring = queryRingtone(firstDetails.name);
                ringtoneChoice = ring;
                }
                /*
                if (!isNumberContact) {       
                String ring = queryRingtone(firstDetails.name);
                ringtoneChoice = ring;
                String  ringTitle = mResources.getString(R.string.ringtone_title);
                ringToneSelector.setText(ringTitle);
                if(ring != null) 
                {
                      Uri uri = Uri.parse(ring);
                      if(RingtoneManager.isDefault(uri))
                      {
                          ring = mResources.getString(R.string.default_ring_tone);
                      } else {
                          int soundId = (int)ContentUris.parseId(uri);
                          if((soundId - 99) >= 0 && soundId - 99 <= 110)
                          {
                          ring = sounds[soundId - 99];
                          }
                      }
                ...
                if(isNumberContact) {
                  /*  ringToneSelector.setTextColor(Color.WHITE);
                    ringToneSelector.setEnabled(false);
                    ringToneSelector.setFocusable(false);
                    ringToneSelector.setClickable(false);*/
                    String  menu_newContact = mResources.getString(R.string.menu_newContact);
                    ringToneSelector.setText(menu_newContact);
                    ringToneSelector.setOnClickListener(new View.OnClickListener() {
                        ...
                        public void onClick(View v) {
                            ...
                            DialerUtils.startActivityWithErrorToast(CallDetailActivity.this,
                                    DialtactsActivity.getAddNumberToContactIntent(displayNumberStr));
                ...
                if (!TextUtils.isEmpty(details.name)) {
                    ...
                    */
                    ...
                    if (callerInfoExt != null) {
                       //qianweiqiang boway return as old version 2015 7 6
                    ...
                    //qianweiqiang set no location return boway 2015 7 6 
    ...
    /// @}
	private void doPickRingtone(String mCustomRingtone) {  
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {  
        ...
        saveData(mCustomRingtone);  
    }  
	private void saveData(String mCustomRingtone) {  
      ContentResolver cr =  CallDetailActivity.this.getContentResolver();
      Uri raw_uri = Uri.parse("content://com.android.contacts/raw_contacts");  
      Cursor cur = cr.query(raw_uri, new String[]{"contact_id"}, "display_name=?", new String[]{mCallerName.getText().toString()+""}, null);
//      Cursor cur = resolver.query(raw_uri, new String[]{"raw_contact_id"}, "contact_id=?", new String[]{contactId+""}, null);
      if(cur != null) 
      {
        cur.moveToFirst();
        int contact_id = cur.getInt(0);
        cur.close();
        ContentValues values = new ContentValues();  
        values.put(Contacts.CUSTOM_RINGTONE, mCustomRingtone);  
        //
        cr.update(Contacts.CONTENT_URI, values, Contacts._ID + " = " + contact_id, null) ;  
      } else {
        cur.close();
       }
	       } 
    ...
    private String queryRingtone(CharSequence name) { 
        ContentResolver cr =  CallDetailActivity.this.getContentResolver();
        Uri raw_uri = Uri.parse("content://com.android.contacts/raw_contacts");  
        Cursor cur = cr.query(raw_uri, new String[]{"custom_ringtone"}, "display_name=?", new String[]{name.toString()+""}, null);
         if(cur != null)
        {
            cur.moveToFirst();
            String ringtoneVoice = cur.getString(0);
            cur.close();
            return ringtoneVoice;
        }
        cur.close();
        return null;
		 }
