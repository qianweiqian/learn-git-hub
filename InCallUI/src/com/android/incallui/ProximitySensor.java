...
//boway modify.20150625.for PS disable issue after power on without cover.begin
                	if(SystemProperties.get(InCallActivity.KEY_LID_CLOSED).equals("true")){
                        TelecomAdapter.getInstance().turnOffProximitySensor(true);
					}else {
						TelecomAdapter.getInstance().turnOnProximitySensor();
					}
//boway modify.20150625.for PS disable issue after power on without cover.end
                }
            } else {
