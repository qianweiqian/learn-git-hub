...
public final class TelecomAdapter implements InCallPhoneListener {
    ...
    public void ignoreCall(String callId, boolean onOff) {
        if (mPhone != null) {
            final android.telecom.Call call = getTelecommCallById(callId);
            if (call != null) {
                android.telecom.TelecomManager.from(mContext).silenceRinger();
            } else {
                Log.e(this, "error ignoreCall, call not in call list: " + callId);
            }
        } else {
            Log.e(this, "error ignoreCall, mPhone is null");
        }
    }
